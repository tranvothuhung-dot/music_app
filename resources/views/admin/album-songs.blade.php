<x-admin-layout title="Bài Hát Theo Album - Admin">
	@php
		$album = $album ?? ['id' => 0, 'name' => ''];
		$songs = collect($songs ?? []);
	...
	<section class="album-songs-page">
		<div class="page-head">
			<h1 class="page-title">Album: {{ $album['name'] }}</h1>
			<a class="back-link" href="{{ route('admin.albums.index') }}">← Quay lại Albums</a>
		</div>

		<div class="per-page-row">
			<form method="GET" action="{{ route('admin.albums.songs', $album['id']) }}" style="display:flex; gap:8px; align-items:center;">
				<label class="per-page-label" for="albumSongsPerPageSelect">Bản ghi mỗi trang:</label>
				<select class="per-page-select" id="albumSongsPerPageSelect" name="per_page" onchange="this.form.submit()">
		...
		@if($songs->isEmpty())
			<div class="empty-box">Album này chưa có bài hát.</div>
		@else
